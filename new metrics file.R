...
#   }
  ...
  for (i in 1:length(stock_list)){
    ...
    axis(1, at=seq(1, length(temp_matrix$DateTime), 390), labels= temp_matrix$DateTime[seq(1, length(temp_matrix$DateTime), 390)], las = 2)
    dev.off()
  }
  ...
  axis(1, at=seq(1, length(temp_matrix$DateTime), 390), labels= temp_matrix$DateTime[seq(1, length(temp_matrix$DateTime), 390)], las = 2)
  legend("bottomleft", c("PortfolioPnL", "MarketPnL"), lty=c(1,1), lwd=c(2.5,2.5),col=c("red","blue"))
  dev.off()
